...
const education = [
  {
    ...
    logo: "/uwaterloo.png",
    courses: ["Fundamentals of Programming (C++)", "Project Studios (MATLAB)", "Digital Circuits (FPGA & Verilog)"],
  },
];
...
const contacts = [
  ...
  { label: "LinkedIn", value: "raymond-fang-214192331", href: "https://www.linkedin.com/in/raymond-fang-214192331", icon: <LinkedinIcon /> },
  { label: "GitHub", value: "rayymondf", href: "https://github.com/rayymondf", icon: <GithubIcon /> },
];
...
// Single Tag component used everywhere — rounded-md, text-xs, consistent sizing
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="px-2.5 py-1 text-xs bg-black/5 dark:bg-white/[0.08] border border-black/8 dark:border-white/[0.10] rounded-md text-black/60 dark:text-white/55 whitespace-nowrap">
      {children}
    </span>
  );
}
...
function ModalSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border border-black/8 dark:border-white/10 rounded-xl p-4 bg-black/[0.02] dark:bg-white/[0.03]">
    ...
    <div className="flex gap-3 mt-6">
      {repo && (
        <a href={repo} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-sm text-black/60 dark:text-white/60 hover:text-black dark:hover:text-white border border-black/15 dark:border-white/15 hover:border-black/35 dark:hover:border-white/35 rounded-full px-4 py-1.5 transition-colors">
...
function BuildBoardModal({ onClose }: { onClose: () => void }) {
  return (
    <ModalShell onClose={onClose}>
      ...
      <p className="text-black/45 dark:text-white/45 text-sm mb-5">Full-Stack Web App · React 19 · Node.js · PostgreSQL · Deployed on Render</p>
      ...
      <div className="space-y-4">
        <p className="text-sm text-black/70 dark:text-white/65 leading-relaxed">
          BuildBoard is a full-stack collaboration platform for student engineers, makers, and robotics teams — a purpose-built alternative to generic social networks for sharing technical projects, recruiting teammates by skill, posting build logs, and messaging collaborators.
        </p>
        <ModalSection title="Architecture">
          <div className="space-y-2">
            <ModalRow label="Frontend" value="React 19 SPA with Vite. Routing via Browser History API. Responsive sidebar nav on desktop, bottom tab bar on mobile." />
            <ModalRow label="Backend" value="Node.js + Express REST API. Session-cookie auth via Passport.js LocalStrategy with bcrypt password hashing." />
            <ModalRow label="Database" value="PostgreSQL on Neon. Parameterized queries throughout. UNIQUE constraints prevent duplicate likes." />
        ...
        <ModalSection title="Key Features">
          <ul className="space-y-1.5 text-sm text-black/65 dark:text-white/60">
            ...
            <li><span className="text-black dark:text-white font-medium">Authorization</span>: owner / admin / member model enforced at the route level</li>
            <li><span className="text-black dark:text-white font-medium">Messaging</span>: DM threads between users with full conversation and message APIs</li>
...
function LingoModal({ onClose }: { onClose: () => void }) {
  return (
    <ModalShell onClose={onClose}>
      ...
      <p className="text-black/45 dark:text-white/45 text-sm mb-5">Chrome Extension · Manifest V3 · DeepL API · Published on Chrome Web Store</p>
      <div className="rounded-xl overflow-hidden border border-black/10 dark:border-white/10 mb-6">
        <Image src="/lingo-ext.png" alt="Lingo extension" width={800} height={500} className="w-full h-auto object-cover" />
      </div>
      <div className="space-y-4">
        <p className="text-sm text-black/70 dark:text-white/65 leading-relaxed">
          Lingo is a Manifest V3 Chrome extension that surfaces a full translation workspace inside Chrome&apos;s native Side Panel. No npm packages, no build step — Chrome loads it directly as an unpacked set of files. Live on the Chrome Web Store.
        </p>
        <ModalSection title="How It Works">
          <div className="space-y-2">
            <ModalRow label="Entry point" value="manifest.json registers a background service worker with sidePanel and storage permissions." />
            <ModalRow label="Service worker" value="Opens the side panel on icon click via chrome.sidePanel.open(), persistent across navigation." />
            <ModalRow label="Panel UI" value="Vanilla JS handles all rendering and event binding via panel.html, styles.css, languageSelector.js, and backendService.js." />
            <ModalRow label="Translation" value="POST /v2/translate to DeepL Free API. Language list fetched from GET /v2/languages with a static fallback." />
            <ModalRow label="Persistence" value="Chrome Storage API saves draft text, last selected language, and translation history across sessions." />
          </div>
        </ModalSection>
        <div className="flex flex-wrap gap-1.5">
...
function TodoModal({ onClose }: { onClose: () => void }) {
  return (
    <ModalShell onClose={onClose}>
      ...
      <p className="text-black/45 dark:text-white/45 text-sm mb-5">Vanilla JavaScript · Zero Dependencies · Three-Layer Architecture</p>
      <div className="rounded-xl overflow-hidden border border-black/10 dark:border-white/10 mb-6">
        <Image src="/todo-app2.png" alt="Todo calendar view" width={800} height={500} className="w-full h-auto object-cover" />
      </div>
      <div className="space-y-4">
        <p className="text-sm text-black/70 dark:text-white/65 leading-relaxed">
          A browser-based task manager with zero dependencies — no framework, no npm, no build step. Enforces a strict three-layer architecture that cleanly separates data logic, storage, and rendering.
        </p>
        <ModalSection title="Architecture">
          <div className="space-y-2">
            <ModalRow label="TodoModel" value="Pure data layer. Handles all CRUD, filtering, sorting, tag queries, calendar date lookups, and overdue detection." />
            <ModalRow label="Storage" value="Serializes state to localStorage under todo-app-v2. Normalizes on load to handle schema changes." />
            <ModalRow label="App" value="Connects model to DOM via event delegation. Manages calendar, list, and three modal types. Escape key closes modals." />
          </div>
        </ModalSection>
        <ModalSection title="Features">
          <ul className="space-y-1.5 text-sm text-black/65 dark:text-white/60">
            <li><span className="text-black dark:text-white font-medium">Calendar view</span>: 7-column monthly grid with todo chips by due date; click any day for a detail modal</li>
            ...
            <li><span className="text-black dark:text-white font-medium">Sort modes</span>: Newest / Due date / Priority / A–Z</li>
            <li><span className="text-black dark:text-white font-medium">Accessibility</span>: ARIA roles, labels, keyboard navigation, programmatic focus management</li>
          </ul>
        </ModalSection>
        <div className="flex flex-wrap gap-1.5">
...
function PoolModal({ onClose }: { onClose: () => void }) {
  return (
    <ModalShell onClose={onClose}>
      ...
      <p className="text-black/45 dark:text-white/45 text-sm mb-5">Java Swing · Custom 2D Physics Engine · OOP Design</p>
      <div className="rounded-xl overflow-hidden border border-black/10 dark:border-white/10 mb-6">
        <Image src="/pool-game.png" alt="Pool game" width={800} height={500} className="w-full h-auto object-cover" />
      </div>
      <div className="space-y-4">
        <p className="text-sm text-black/70 dark:text-white/65 leading-relaxed">
          Two-player 8-ball billiards in Java Swing with a fully custom physics engine — all collision detection, resolution, and movement dynamics computed from scratch on a Swing Timer game loop, no third-party physics library.
        </p>
        <ModalSection title="Physics Engine">
          <div className="space-y-2">
            <ModalRow label="Integration" value="Per-tick velocity integration. Positions updated by velocity × dt. Friction decelerates multiplicatively until below a rest threshold." />
            <ModalRow label="Ball collisions" value="Elastic circle-circle overlap detection. Velocities resolved along the collision normal via conservation of momentum." />
            <ModalRow label="Cushion bounce" value="AABB checks on all four rails. Normal component reflected and scaled by an energy loss coefficient." />
            <ModalRow label="Sub-step CCD" value="Physics step subdivided at high speeds to prevent balls from tunnelling through each other or the rails." />
          </div>
        </ModalSection>
        <div className="flex flex-wrap gap-1.5">
...
function PCBModal({ onClose }: { onClose: () => void }) {
  return (
    <ModalShell onClose={onClose}>
      ...
      <p className="text-black/45 dark:text-white/45 text-sm mb-5">UW Orbital · Altium Designer · Analog Hardware Design</p>
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="rounded-xl overflow-hidden border border-black/10 dark:border-white/10 flex flex-col">
          ...
          <p className="text-center text-xs text-black/40 dark:text-white/40 py-1.5">PCB Layout</p>
        </div>
        <div className="rounded-xl overflow-hidden border border-black/10 dark:border-white/10 flex flex-col">
          <div className="relative w-full bg-white dark:bg-white/5" style={{ aspectRatio: "4/3" }}>
            <Image src="/pcb2.png" alt="Schematic" fill className="object-contain p-2" />
          </div>
          <p className="text-center text-xs text-black/40 dark:text-white/40 py-1.5">Schematic</p>
        </div>
      </div>
      <div className="space-y-4">
        <p className="text-sm text-black/70 dark:text-white/65 leading-relaxed">
          A low-side current-sense breakout board for UW Orbital&apos;s Electrical Power System. Measures DC bus current on a 5V rail over 0–200 mA and outputs a proportional analog voltage readable by an MCU ADC.
        </p>
        <ModalSection title="Design Overview">
          <div className="space-y-2">
            <ModalRow label="Topology" value="Low-side sensing — shunt between load return and ground. Both amplifier inputs operate near ground, avoiding high common-mode voltage." />
            <ModalRow label="Amplifier" value="INA180B3IDBVR (100 V/V). At 200 mA through a 10 mΩ shunt, the 2 mV differential is amplified to 200 mV — well within a 3.3V ADC range." />
            <ModalRow label="Layout" value="Altium Designer on Orbital's shared Altium 365 workspace. GND copper pour on both layers, stitched with vias." />
          </div>
        </ModalSection>
      </div>
    </ModalShell>
...
function WATonomousModal({ onClose }: { onClose: () => void }) {
  return (
    <ModalShell onClose={onClose}>
      ...
      <p className="text-black/45 dark:text-white/45 text-sm mb-5">WATonomous ASD · ROS2 Humble · C++17 · Gazebo · Foxglove</p>
      <div className="rounded-xl overflow-hidden border border-black/10 dark:border-white/10 mb-6">
        <Image src="/wato-asd.png" alt="Foxglove visualization" width={800} height={500} className="w-full h-auto object-cover" />
        <p className="text-center text-xs text-black/40 dark:text-white/40 py-1.5">Foxglove: live costmap, inflated obstacles, and A* path overlaid on the occupancy grid</p>
      </div>
      <div className="space-y-4">
        <p className="text-sm text-black/70 dark:text-white/65 leading-relaxed">
          Full autonomous navigation stack for a simulated differential-drive robot in Gazebo. Four tightly coupled ROS2 C++ nodes implement the full perception–world model–planning–control pipeline used in production autonomous systems.
        </p>
        <ModalSection title="Node Architecture">
          <ul className="space-y-3 text-sm text-black/65 dark:text-white/60">
            <li><span className="text-black dark:text-white font-medium">Costmap</span>: Converts LiDAR scan to Cartesian grid, marks occupied cells, applies inflation kernel. Publishes <code className="text-xs bg-black/8 dark:bg-white/10 px-1 rounded">OccupancyGrid</code>.</li>
            <li><span className="text-black dark:text-white font-medium">Map Memory</span>: Fuses local costmaps into a persistent global map via odometry. Updates only beyond a displacement threshold.</li>
            <li><span className="text-black dark:text-white font-medium">Planner (A*)</span>: A* with Euclidean heuristic, idle/tracking state machine, replans on map updates. Publishes <code className="text-xs bg-black/8 dark:bg-white/10 px-1 rounded">nav_msgs/Path</code>.</li>
            <li><span className="text-black dark:text-white font-medium">Controller (Pure Pursuit)</span>: Selects lookahead waypoint, computes arc curvature, outputs <code className="text-xs bg-black/8 dark:bg-white/10 px-1 rounded">geometry_msgs/Twist</code> at 10 Hz.</li>
          </ul>
        </ModalSection>
        <div className="flex flex-wrap gap-1.5">
    ...
    >
      ...
      <div className="flex flex-col gap-2.5 p-4 flex-1">
        ...
        <button onClick={onLearnMore}
          className="self-start mt-1 text-sm text-black/50 dark:text-white/50 hover:text-black dark:hover:text-white border border-black/15 dark:border-white/15 hover:border-black/35 dark:hover:border-white/35 rounded-full px-3 py-1 transition-colors">
          Learn More
        </button>
      </div>
    </motion.div>
    ...
    <>
        ...
        {education.map((e, i) => (
          <motion.div key={e.school} variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} custom={i}
            ...
            <div className="mt-4 pt-4 border-t border-black/8 dark:border-white/8">
              ...
              <div className="flex flex-wrap gap-1.5">
                {e.courses.map((course) => <Tag key={course}>{course}</Tag>)}
              </div>
            </div>
          </motion.div>
        ))}
        ...
        <div className="relative">
          ...
          <div className="space-y-6">
            {experience.map((e, i) => (
              ...
              >
                <div className="shrink-0 z-10" style={{ width: 56 }}>
                  <div className="w-14 h-14 rounded-xl overflow-hidden bg-white border border-black/8 dark:border-white/10 flex items-center justify-center">
      ...
      <Section id="projects" title="Projects">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-stretch">
          {projects.map((p, i) => (
            <ProjectCard key={p.name} project={p} index={i} onLearnMore={() => setOpenModal(p.modalKey)} />
          ))}
        </div>
      </Section>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {contacts.map(({ label, value, href, icon }, i) => (
            <motion.a key={label} href={href}
              ...
              variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} custom={i}
              whileHover={{ x: 4 }}
              className="flex items-center gap-4 border border-black/10 dark:border-white/12 rounded-xl p-4 hover:border-black/30 dark:hover:border-white/30 hover:bg-black/[0.03] dark:hover:bg-white/[0.05] transition-all group">
      ...
      <footer className="py-10 px-8 border-t border-black/8 dark:border-white/8 bg-[#f5f5f0] dark:bg-[#0a0a0a]">
