...
export const experience: ExperienceEntry[] = [
  {
    ...
    bullets: [
      "trained ACT + diffusion policies on 200+ demos, hitting 90% grasp-and-place success across 50 sim + hardware trials",
      "cut policy training time 65% with gpu-parallel Isaac Lab (2000+ envs, domain randomization), then shipped it: ONNX → TensorRT, 50 inferences/s onboard in ROS 2",
      "also wrote the firmware underneath: FOC motor control on an STM32G4, 1kHz FreeRTOS loop over 6 joints, micro-ROS over CAN-FD at <2ms",
      "fine-tuned a VLA model on annotated demo trajectories so the arm follows spoken instructions, 8+ new tasks with zero new demos",
    ],
  },
  {
    company: "einfolab",
    ...
    bullets: [
      ...
      "caught errors sql rules missed: Isolation Forest (92% precision) + RAG-backed LLM classifier (LangChain, FAISS), cutting manual review from 2h to 30min",
      "shipped versioned FastAPI endpoints exposing pipeline run status, error logs, and audit trails, giving stakeholders real-time visibility into data quality",
    ],
  },
  {
    ...
    bullets: [
      "c/c++ firmware for the battery, vehicle control, and power distribution units: 12+ CAN signals with fail-safe logic, zero unplanned shutdowns in 30+ track sessions",
      "live telemetry dashboard (React + FastAPI, 5000+ CAN frames per test over WebSocket) with a battery anomaly model at 94% accuracy",
      "led an FMEA of the high-voltage fault paths and redesigned the ad-hoc logic into a clean state machine (startup / drive / charge / fault), cutting fault-diagnosis time roughly in half",
      "HIL test rig in c++/python: 30+ fault scenarios, caught 10+ bugs before they reached the car",
    ],
  },
  {
...
export const links = {
  ...
  resume: "https://drive.google.com/file/d/1kOq3bsIyUaNYXR__M44FkwblHbUEdepC/view?usp=sharing",
};
